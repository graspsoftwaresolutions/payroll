...
<div class="content-wrapper">
  ...
  <section class="content-header">
    ...
    <ol class="breadcrumb">
      ...
      <li class="active">{{ __('Yearly Report') }}</li>
    </ol>
  </section>
  ...
  <section class="content">
    ...
    <div class="box">
      <div class="box-header with-border">
        <h3 class="box-title">{{ __('Yearly Report') }}</h3>
          ...
          <form class="form-horizontal" id="searchform" action="{{ url('/hrm/epfsocso/statement') }}" method="post">
            ...
            <div class="form-group{{ $errors->has('user_id') ? ' has-error' : '' }} ">
              ...
              <div class="form-group{{ $errors->has('salary_month') ? ' has-error' : '' }}">
                <label for="salary_month" class="col-sm-3 control-label">{{ __('Select Year') }}</label>
                <div class="col-sm-3">
                  <select id="reportyear" name="reportyear" class="form-control">
                    @for($y=2010;$y<=date('Y');$y++)
                      <option @if(date('Y')== $y) selected @endif value="{{ $y }}"> {{ $y }}</option>
                    @endfor
                  </select>
              ...
              <div class="form-group">
                <div class="col-sm-offset-3 col-sm-10">
                  <button type="submit" class="btn btn-primary btn-flat"><i class="fa fa-arrow-right"></i> {{ __('VIEW YEARLY REPORT') }}</button>
                </div>
              </div>
              <!-- /.end group -->
            </form>
  ...
  @endsection
  @php
    $redirecturl = url('hrm/yearlydeductions_print');
  @endphp
  ...
  @section('footerSection')
    ...
    <script type="text/javascript">
      $("#searchform").submit(function(e){
          ...
          if(reportyear!='' && user_id!=''){
            win = window.open('{{ $redirecturl }}?reportyear='+reportyear+'&user_id='+user_id, '_blank');
          }
          return false;
      });
