...
<div class="content-wrapper">
  ...
  <section class="content-header">
    ...
    <ol class="breadcrumb">
      ...
      <li><a>{{ __('Other sDeduction') }}</a></li>
      <li class="active">{{ __('Manage Edit Other Deduction') }}</li>
    </ol>
  </section>
  ...
  <section class="content">
    ...
    <div class="box">
      <div class="box-header with-border">
        <h3 class="box-title">{{ __('Manage Edit Other Deduction') }}</h3>
      ...
      <div class="box-body">
        ...
        <div class="col-md-12">
            ...
            @php $row = $data['deduction_data'][0]; @endphp
          <form class="form-horizontal" action="{{ route('otherdeduction.update') }}" method="post">
            {{ csrf_field() }}
            <input type="hidden" name="deduction_id" id="deduction_id" value="{{$row->id}}">
            <div class="form-group{{ $errors->has('name') ? ' has-error' : '' }}">
              <label for="name" class="col-sm-3 control-label">{{ __('Deduction name') }}<span style="color:red"> *</span></label>
              <div class="col-sm-6">
               <input type="text" name="name" class="form-control" value="{{$row->name}}"  placeholder="{{ __('Other Deduction name') }}">
               @if ($errors->has('name'))
            ...
            <div class="form-group{{ $errors->has('cat_id') ? ' has-error' : '' }}">
              ...
              <div class="col-sm-6">
                <select name="cat_id" id="cat_id" class="form-control">
                  <option value="0" disabled selected="true">{{ __('Select category') }}</option>
              ...
              <div class="form-group">
                <div class="col-sm-offset-3 col-sm-10">
                  <button type="submit" class="btn btn-primary btn-flat"><i class="fa fa-arrow-right"></i>{{ __('Update Deduction') }} </button>
                </div>
              </div>
              <!-- /.end group -->
            </form>
